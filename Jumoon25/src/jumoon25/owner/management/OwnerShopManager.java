...
public class OwnerShopManager {
	...
	public ModelAndView owner_shop_management(HttpSession session, ModelAndView mav) {
		...
		int check = (Integer) sqlMap.queryForObject("owner.ownerCheckPermission", session.getAttribute("ownerId"));
		if (check == 0) {
			mav.setViewName("/owner_management/owner_checkPermission");
		} else {
			String id = (String)session.getAttribute("ownerId");
			String table_name = id + "_shop";
			check = (int) sqlMap.queryForObject("shop.shopTableCheck", table_name.toUpperCase());
			if (check == 0) {
